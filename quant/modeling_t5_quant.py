...
class T5DenseReluDense(nn.Module):
    def __init__(self, config):
        ...
        self.wi = QuantizeLinear(config.d_model, config.d_ff, config=config)
        # self.wo = nn.Linear(config.d_ff, config.d_model, bias=False)
        self.wo = QuantizeLinear(config.d_ff, config.d_model, config=config)
...
class T5DenseGatedGeluDense(nn.Module):
    def __init__(self, config):
        ...
        self.wi_0 = QuantizeLinear(config.d_model, config.d_ff, config=config)
        # self.wi_1 = nn.Linear(config.d_model, config.d_ff, bias=False)
        self.wi_1 = QuantizeLinear(config.d_model, config.d_ff, config=config)
        # self.wo = nn.Linear(config.d_ff, config.d_model, bias=False)
        self.wo = QuantizeLinear(config.d_ff, config.d_model, config=config)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.q = QuantizeLinear(self.d_model, self.inner_dim, config=config)
        self.k = QuantizeLinear(self.d_model, self.inner_dim, config=config)
        self.v = QuantizeLinear(self.d_model, self.inner_dim, config=config)
        self.o = QuantizeLinear(self.inner_dim, self.d_model, config=config)
    ...
    def forward(
        ...
        # quantize both attention weights and value states for dot product
        if self.quantize_act:
            attn_weights = self.act_quantizaer.apply(attn_weights, self.clip_attn, self.input_bits, True)
